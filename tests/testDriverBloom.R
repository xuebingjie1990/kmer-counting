...
source("/home/nsheff/code/compbio_program/COBI_computational_genomics_course/kmer_counting/bloomFilter.R")

message(check, " Found bloom filter code")

bfilt = resetFilter(bfilt, 100000)
bfilt = addElement("ACTGT", bfilt)

assert("Basic insertion", {
	(testElement("ACTGT") == TRUE)
	(testElement("TCGAT") == FALSE)
})
...
for (kmer in randomKmers) {
	bfilt = addElement(kmer)
}

# True positive
truePos = sum(sapply(randomKmers, testElement)) / length(randomKmers)

# False positive
falsePos = sum(sapply(randomKmers2, testElement)) / length(randomKmers2)

# Full rate
fullRate = sum(bfilt)/length(bfilt)
